...
pub fn findClosestPointOnQuadraticBezier(
    ...
    for (roots) |t| {
        if (t >= 0 and t <= 1) {
            ...
            const dist = render.calcDist(point.x, point.y, pt.x, pt.y);
            if (dist < min_dist) {
                min_dist = dist;
                best_t = t;
            }
    ...
    const dist0 = render.calcDist(point.x, point.y, p0.x, p0.y);
    if (dist0 < min_dist) {
        min_dist = dist0;
        best_t = 0;
    }

    const dist1 = render.calcDist(point.x, point.y, p2.x, p2.y);
    if (dist1 < min_dist) {
        min_dist = dist1;
        best_t = 1;
    }
